...
KhulnaSoft AI SDK provides a set of utilities to make it easy to use Anthropic's API. In this guide, we'll walk through how to use the utilities to create a chat bot and a text completion app.
...
<Callout>
  ...
  First, we pass the streaming `response` we receive from Anthropic's TypeScript
  SDK to [`AnthropicStream`](/docs/api-reference/anthropic-stream). This utility
