...
Here is an example implementation of a chat application that uses both Khulnasoft AI SDK and a composed LangChain chain together with the
...
It takes this stream and uses Khulnasoft AI SDK's [`StreamingTextResponse`](/docs/api-reference/streaming-text-response)
to pipe text to the client (from the edge) and then Khulnasoft AI SDK's `useChat` to handle the chat UI.
...
Then, we use the Khulnasoft AI SDK's [`useChat`](/docs/api-reference/use-chat) method:
