...
Khulnasoft AI SDK provides a set of utilities to make it easy to use [Perplexity](https://perplexity.ai)'s APIs and models. In this guide, we'll walk through how to use the utilities to create a chat bot.
...
Create a Next.js application and install `ai` and `openai`, the Khulnasoft AI SDK and OpenAI API client respectively. Perplexity's REST APIs are compatible with OpenAI's so we will use OpenAI's JavaScript SDK to make the requests.
...
<Callout>
  Khulnasoft AI SDK provides 2 utility helpers to make the above seamless: First, we
